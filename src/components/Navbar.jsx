      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            /* 
              PERBAIKAN KUNCI:
              1. Digunakan h-[calc(100dvh-4rem)] menggunakan dvh (Dynamic Viewport Height).
              2. top-16 (4rem) menjamin offset presisi di bawah navbar h-16.
              3. overflow-y-auto memastikan seluruh isi menu dapat di-scroll jika layar terlalu pendek.
            */
            className="md:hidden fixed top-16 left-0 right-0 h-[calc(100dvh-4rem)] w-full bg-zinc-950 border-t-4 border-black px-5 py-6 flex flex-col justify-between overflow-y-auto z-[99]"
          >
            {/* Header Status Menu */}
            <div className="flex-shrink-0 flex items-center justify-between border-b-2 border-dashed border-zinc-800 pb-3 mb-4">
            ...
            <div className="flex flex-col gap-2.5 my-auto py-2">
              {navLinks.map((link, idx) => (
                <motion.a
                  ...
                  className="group bg-zinc-900 border-2 border-black p-3.5 flex items-center justify-between shadow-[3px_3px_0px_0px_#000] active:translate-x-0.5 active:translate-y-0.5 hover:bg-han-green transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <span className="font-pixel text-[9px] text-zinc-500 group-hover:text-black font-bold">
                      //{link.code}
                    </span>
                    <span className="font-retro text-xl uppercase tracking-wider text-white group-hover:text-black font-bold">
                      {link.label}
                    </span>
                  </div>
            ...
            <motion.div
              ...
              className="flex-shrink-0 pt-4 mt-4 border-t-2 border-dashed border-zinc-800"
            >
              <a
                href="#contact"
                onClick={handleLinkClick}
                className="w-full bg-han-green text-black border-4 border-black py-3.5 font-pixel text-[10px] sm:text-[11px] uppercase tracking-widest text-center font-black flex items-center justify-center gap-2 shadow-[4px_4px_0px_0px_#000] active:translate-x-0.5 active:translate-y-0.5 transition-all"
              >
